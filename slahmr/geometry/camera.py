...
def reproject(points3d, cam_R, cam_t, intrins, cam_dist):
    """
    Reproject 3D points into each camera of a calibrated multiview rig, applying
    the OpenCV radial/tangential distortion model.

    :param points3d (1, T, N, 3) predicted 3D points for the single fitted subject
    :param cam_R (V, T, 3, 3) world-to-camera rotation, per view per frame
    :param cam_t (V, T, 3) world-to-camera translation, per view per frame
    :param intrins (V, T, 3, 3) camera intrinsics matrix, per view per frame
    :param cam_dist (V, T, 5) OpenCV distortion coeffs (k1, k2, p1, p2, k3), per view per frame
    :returns (V, T, N, 2) reprojected pixel coordinates in each view
    """
    ...
    tangential_xy = 2 * cam_dist[:, :, None, 2:4] * points2d[..., [0]] * points2d[..., [1]]
    tangential_r2 = cam_dist[:, :, None, [3, 2]] * (r2 + 2 * points2d**2)
    points2d = points2d * radial + tangential_xy + tangential_r2
    points2d = (
        torch.einsum("vtij,vtnj->vtni", intrins[:, :, :2, :2], points2d)
        + intrins[:, :, None, :2, 2]
    )
    return points2d
