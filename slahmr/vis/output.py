...
def prep_result_vis(res, vis_mask, track_ids, body_model):
    ...
    res = detach_all(res)
    # use the first camera view as the reference camera for visualization purposes
    res["cam_R"] = res["cam_R"][[0]]
    res["cam_t"] = res["cam_t"][[0]]
    res["intrins"] = res["intrins"][[0]]
    res["cam_dist"] = res["cam_dist"][[0]]
    with torch.no_grad():
        world_smpl = run_smpl(
            body_model,
            res["trans"],
...
def build_scene_dict(world_smpl, vis_mask, track_ids, T_w2c=None, **kwargs):
    scene_dict = {}
